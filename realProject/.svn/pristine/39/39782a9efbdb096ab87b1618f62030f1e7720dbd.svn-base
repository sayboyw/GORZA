...
public class CodyPaging {
	private int totalNum; 		// 전체 글의 개수 dao에서 전체글 count
	private int pageNum; 		// 현재 페이지 번호
	private int listCount; //10		// 페이지당 나타낼 글의 갯수 (페이지 수 지정 )
	private int pageCount; //2		// 페이지그룹당 페이지 갯수
	private String boardName; 	// 게시판의 종류 url 담을꺼임
	private int num; // 구분자
	
	public CodyPaging(int maxNum, int pageNum, int listCount, int pageCount, String boardName,int num) {
	...
	private String makeHtml(int currentGroup, int totalPage, String boardName) {
		...
		if (start != 1) {
			sb.append("<a onclick='searchMethod(sit,color,brand,divide,"+num+","+(start-1)+")' href='#'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a onclick='searchMethod(sit,color,brand,divide,"+num+","+(i)+")' href='#'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='#' onclick='searchMethod(sit,color,brand,divide,"+num+","+(end+1)+")'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
